...
The ten-year interface progressively requests `/api/plan/simulate` in bounded batches. The first nonzero frame may wait for a monthly response cache; measure this cold start on Vercel. Sampling and learning are full-history operations despite being delivered through separate requests.
