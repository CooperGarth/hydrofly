...
export function createMineScene(container){
 ...
 let lastFloor=null,lastWells=null,lastRates=null;
 ...
 function update(result){consoleWorld.updateModel(result);const wellsKey=JSON.stringify(result.wells),ratesKey=JSON.stringify(result.rates[result.active_year]);if(lastFloor!==result.floor||lastWells!==wellsKey||lastRates!==ratesKey){geology(result.floor,result.wells,result.rates[result.active_year]);lastFloor=result.floor;lastWells=wellsKey;lastRates=ratesKey;}clear(headGroup);const axis=result.axis,N=axis.length,v=[],col=[],ix=[];for(let j=0;j<N;j++)for(let i=0;i<N;i++){const h=result.surface[j][i];v.push(axis[i],y(h),axis[j]);const c=new THREE.Color(0xddb97b).lerp(new THREE.Color(0x78c5bf),THREE.MathUtils.clamp((h-result.target+25)/60,0,1));col.push(c.r,c.g,c.b);}for(let j=0;j<N-1;j++)for(let i=0;i<N-1;i++){const k=j*N+i;ix.push(k,k+N,k+1,k+1,k+N,k+N+1);}const geo=new THREE.BufferGeometry();geo.setAttribute('position',new THREE.Float32BufferAttribute(v,3));geo.setAttribute('color',new THREE.Float32BufferAttribute(col,3));geo.setIndex(ix);geo.computeVertexNormals();headGroup.add(new THREE.Mesh(geo,new THREE.MeshStandardMaterial({vertexColors:true,side:THREE.DoubleSide,transparent:true,opacity:.55,depthWrite:false,roughness:.4})));
