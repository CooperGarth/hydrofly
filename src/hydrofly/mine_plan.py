...
class MinePlan(BaseModel):
    ...
    rates:list[list[float]]=Field(default_factory=lambda:[row.copy() for row in DEMO_RATES])
    objective:Literal['cost','rate','drawdown']='drawdown'
    conductivity:float=Field(default=.05,ge=.0001,le=5)
    thickness:float=Field(default=200,ge=100,le=400)
    storativity:float=Field(default=.01,ge=.0001,le=.01)
    capacity:float=Field(default=6000,ge=100,le=10000)
    view_extent:float=Field(default=10000,ge=2600,le=20000)
    initial_head:float=Field(default=-355,ge=-899,le=360)
    ...
    def aquifer(self):return Aquifer(self.conductivity,self.thickness,self.storativity,self.initial_head,-900)
    def wells(self):
...
def objective_metric(plan,q,h):
    ...
    return float(np.mean(np.maximum(targets-h[:,:25],0)/np.maximum(plan.initial_head-targets,1)))

def cost(plan,h=None):
    h=numerical(plan) if h is None else h
    targets=target_heads(plan)
    scale=np.maximum(plan.initial_head-targets,1)
...
def simulation_frames(request):
    ...
    return {'frames':frames,'next':request.start+len(frames),'done':request.start+len(frames)>n,
        ...
        'initial_head':p.initial_head,'initial_floor':p.initial_floor,'years':[{'year':i+1,'floor':h,'target':h-5} for i,h in enumerate(p.floors)]}
